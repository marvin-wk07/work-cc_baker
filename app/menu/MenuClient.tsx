  ...
  return (
    <div>
        ...
        <button
          ...
          className={`px-5 py-2 rounded-full font-medium text-sm transition-colors ${
            view === 'shop'
              ...
              : 'bg-white text-stone-600 border border-amber-200 hover:bg-orange-100'
          }`}
        >
          🛍️ 選購商品
        </button>
      ...
      {view === 'image' ? (
        ...
        <div>
          {/* Category Filter */}
          <div className="flex flex-wrap gap-2 mb-8 sticky top-[57px] bg-orange-100 py-3 z-10 -mx-4 px-4 border-b border-amber-100">
            {categories.map(cat => (
              <button
                key={cat}
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
                  activeCategory === cat
                    ? 'bg-amber-400 text-white shadow-sm'
                    : 'bg-white text-stone-600 hover:bg-orange-100 border border-amber-200'
                }`}
              >
                {cat}
              </button>
